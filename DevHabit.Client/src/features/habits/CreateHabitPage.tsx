...
import { AutomationSource, FrequencyType, Habit, HabitType } from "./types";
...
export const CreateHabitPage: React.FC = () => {
	...
	const handleInputChange = (
		...
		if (name === "type") {
			...
			if (
				newType === HabitType.Binary &&
				!BINARY_UNITS.includes(formData.target.unit as any)
			) {
				setFormData((prev) => ({
					...prev,
					target: {
				...
				<div className="grid grid-cols-2 gap-4">
					...
					<div>
						...
						<select
							...
							className="w-full px-3 py-2 border rounded-md"
						>
							{(formData.type === HabitType.Binary
								? BINARY_UNITS
								: ALLOWED_UNITS
							).map((unit) => (
